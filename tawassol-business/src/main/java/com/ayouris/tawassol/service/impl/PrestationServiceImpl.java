...
public class PrestationServiceImpl extends GenericServiceImpl<Prestation, Long> implements PrestationService {
    ...
    @Override
    public List<PrestationBean> getActiveLivre() {
        QPrestation prestation = QPrestation.prestation;
        Iterable<Prestation> prestations = prestationRepository.findAll(prestation.active.isTrue().and(prestation.livree.isTrue()));
        return mapper.map(prestations, PrestationBean.LIST_BEAN_TYPE);
    }
