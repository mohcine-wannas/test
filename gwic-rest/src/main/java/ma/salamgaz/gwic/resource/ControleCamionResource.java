...
@Component("controleCamionResource")
@Path("/controleCamion")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class ControleCamionResource {

    private static final Logger LOGGER = LoggerFactory.getLogger(ControleCamionResource.class);

    @Autowired
    private ControleCamionService controleCamionService;

    @Autowired
    public ControleCamionResource(){
    ...
    public Response list(PageDataBean pageDataBean){
        return Response.ok(this.controleCamionService.list(pageDataBean)).build();
    }

    @GET
    @Path("{id}")
    public Response getDetails(@PathParam("id") Long id) {
        return Response.ok(this.controleCamionService.getDetails(id)).build();
    }

    @DELETE
    @Path("{id}")
    public Response remove(@PathParam("id") Long id) {
        this.controleCamionService.delete(id);
        return Response.ok().build();
    }

    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response create(ControleCamionBean object) {
        try {

            this.controleCamionService.createOrUpdate( object);
            return Response.ok().build();
    ...
    public Response getColumnDef() {
        try {
            return Response.ok(columnDefService.getByViewName(ViewName.CONTROLE_CAMION)).build();
        } catch (ServiceException e) {
            LOGGER.error(e.getMessage());
    ...
    public Response getAll() {
        return Response.ok(this.controleCamionService.getAll()).build();
    }
