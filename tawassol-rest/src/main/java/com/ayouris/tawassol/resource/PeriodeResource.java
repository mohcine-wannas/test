...
@Component("periodeResource")
@Path("/periode")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class PeriodeResource {

    private static final Logger LOGGER = LoggerFactory.getLogger(PeriodeResource.class);

    @Autowired
    private PeriodeService periodeService;

    @Autowired
    public PeriodeResource(){
    ...
    public Response list(PageDataBean pageDataBean){
        return Response.ok(this.periodeService.list(pageDataBean)).build();
    }

    @GET
    @Path("{id}")
    public Response getDetails(@PathParam("id") Long id) {
        return Response.ok(this.periodeService.getDetails(id)).build();
    }

    @DELETE
    @Path("{id}")
    public Response remove(@PathParam("id") Long id) {
        this.periodeService.delete(id);
        return Response.ok().build();
    }

    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response create(PeriodeBean object) {
        try {

            this.periodeService.createOrUpdate( object);
            return Response.ok().build();
    ...
    public Response getAll() {
        return Response.ok(this.periodeService.getAll()).build();
    }
