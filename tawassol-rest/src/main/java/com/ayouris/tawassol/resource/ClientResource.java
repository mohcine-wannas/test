...
public class ClientResource {
    ...
    public Response list(PageDataBean pageDataBean){
        return Response.ok(this.clientService.list(pageDataBean)).build();
    }

    @GET
    @Path("{id}")
    public Response getDetails(@PathParam("id") Long id) {
        return Response.ok(this.clientService.getDetails(id)).build();
    }
    ...
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response create(ClientBean object) {
        try {
            this.clientService.createOrUpdate( object);
            return Response.ok().build();
    ...
    public Response getColumnDef() {
        try {
            return Response.ok(columnDefService.getByViewName(ViewName.CLIENT)).build();
        } catch (ServiceException e) {
            LOGGER.error(e.getMessage());
    ...
    public Response getAll() {
        return Response.ok(this.clientService.getAll()).build();
    }
    
    @GET
    @Path("/{id}/concessionnaires/")
    @JsonView(ListView.class)
    public Response getAllConcessionnairesByClient(@PathParam("id") Long id) {
        return Response.ok(this.clientService.getAllConcessionnairesByClientId(id)).build();
    }
