...
import com.ayouris.tawassol.common.model.bean.DateFilterValue;
...
//				if (dataAbstractFilter instanceof DataFilterBean) {
//					DataFilterBean dataFilter = (DataFilterBean) dataAbstractFilter;
				try {
					...
					if(columnDef == null) {
						System.out.println("Filter column : " + column + " not found");
						continue;
					}
					...
					if (columnDef != null) {
						...
						case TEXT:
							...
							StringPath StringExp = (StringPath) f.get(o);
							String value = dataFilter.getStringValue();
							if (value.contains(",")){
								String[] elem = value.split(",");
								List<String> items = Arrays.asList(elem);
								for (int i = 0; i < items.size(); i++){
									String val = items.get(i);
									item = StringExp.equalsIgnoreCase(val);
									currentMatchFilter = currentMatchFilter == null ? item : currentMatchFilter.or(item);
								}
							}
							else {
						...
						| SecurityException | InvocationTargetException | NoSuchMethodException | ParseException e) {
					e.printStackTrace(); // TODO Logger or throw
				}
